...
function AfficheClient($clients,$types,$filtre){
	...
	$code.="
	&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<button class='btn btn-default'><i class='fa fa-refresh'></i> Actualiser</button>
	</form>
	&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
	...
	<th>Nom Client <i class='fa fa-sort' id='alphaSort'></i></th>
...
function AfficheFicheClient($client,$types_client,$conseillers,$civilites,$situations,$sensibilites,$categories,$raisons,$professions,$status,$types_revenus,$revenus,$types_historique,$historiques,$types_relation,$relations,$personnes,$besoins,$occurences,$besoins_cli,$type_produits,$compagnies,$produits,$anom1,$anom2){
	$code='
	<h4 style="display:inline;">'.$client["CLT-Nom"].' '.$client["CLT-Prénom"].'</h4>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<h5 style="display:inline;"><i>'.Auth::getInfo('ongletTitre').'</i></h5>
	&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
	...
	if($_GET['onglet'] == "solution"){
		$code.=AfficheFicheClientSolution($client,$type_produits,$compagnies,$produits,$anom1,$anom2);
	}
		...
		<form method="post" action="index.php?action=modifClientPersonnel" class="form-horizontal">
			...
			<div class="col-lg-6">
				  <div class="panel panel-primary" style="background-color:#EBEAE8;">
		            ...
		            <div class="panel-body">
						<label style="left:-7px;">Date Naissance : </label> 
						<input type="date" name="dateNaissance" style="width:120px;height:22px;" value="';
					if($client['CLT-DateNaissance']!=null){
						$code.=date('Y-m-d',strtotime($client['CLT-DateNaissance']));
					} else {
						$code.="";
					}
					$code.='"/><br/><br/>
	...
	$code='<div class="tab-pane fade in';
		...
		foreach($revenus as $revenu) { 
			...
			<tr><td><form method="post" action="index.php?action=modifClientRevenu" style="display:inline;">
				...
				$code.='</select>
				<td><input type="text" class="form-control" name="annee" style="width:100px;" value="'.$revenu['R/C-Année'].'" required/></td>
				<td><div class="form-group"><div class="input-group"><div class="input-group-addon">€</div><input class="form-control" type="text" name="montant" style="width:100px;" value="'.$revenu['R/C-Montant'].'" required/></div></div></td>
				<td><button type="submit" class="btn btn-warning btn-xs"><i class="fa fa-pencil fa-lg"></i> Enregistrer</button></td>
...
function AfficheFicheClientHistorique($client,$types_historique,$historiques){
	$code='<div class="tab-pane fade in';
		...
		foreach ($historiques as $historique) {
			...
			$code.='</select></td>
			<td>
			<input type="date" name="date" style="width:120px;height:22px;"value="';
			if($historique['H/C-Date']!=null){
				$code.=date('Y-m-d',strtotime($historique['H/C-Date']));
			} else {
				$code.="";
			}
			$code.='"/></td>
			<td><input type="date" name="echMax" style="width:120px;height:22px;" value="';
			if($historique['H/C-DateMax']!=null){
				$code.=date('Y-m-d',strtotime($historique['H/C-DateMax']));
			} else {
				$code.="";
			}
			...
			$code.='</td>
			<td><input type="date" name="dateCloture" style="width:120px;height:22px;" value="';
			if($historique['H/C-DateCloture']!=null){
				$code.=date('Y-m-d',strtotime($historique['H/C-DateCloture']));
			} else {
				$code.="";
			}
			$code.='"/></td>
	...
	$code.=' 
		...
		<form method="post" action="index.php?action=addClientHistorique" id="formHistorique">
			...
			$code.='</select></td>
			<td><input type="date" name="date" style="width:120px;height:22px;"/></td>
			<td><input type="date" name="echMax" style="width:120px;height:22px;"/></td>
			...
			<td><input type="date" name="dateCloture" style="width:120px;height:22px;"/></td>
...
function AfficheFicheClientRelationel($client,$type_relation,$relations,$personnes){
	...
	<div class="tab-pane fade in';
		...
		<div id="ajoutLienType" style="display:none;">
	...
	<div class="table-responsive">
		...
		<tbody>';
			...
			foreach ($produits2 as $produit) {
				$code.='<tr>';
				$code.="<td onclick=\"window.open('index.php?action=ficheClientProduit&idProduit=".$produit['P/C-NumID']."');\" class=\"rowClient\">".$produit['CLT-Nom']." ".$produit['CLT-Prénom']."</td>";
				$code.="<td onclick=\"window.open('index.php?action=ficheClientProduit&idProduit=".$produit['P/C-NumID']."');\" class=\"rowClient\">".$produit['CIE-Nom']."</td>";
				$code.="<td onclick=\"window.open('index.php?action=ficheClientProduit&idProduit=".$produit['P/C-NumID']."');\" class=\"rowClient\">".$produit['PDT-Nom']."</td>";
				$code.="<td onclick=\"window.open('index.php?action=ficheClientProduit&idProduit=".$produit['P/C-NumID']."');\" class=\"rowClient\">".$produit['TSC-Nom']."</td>";
				$code.="<td onclick=\"window.open('index.php?action=ficheClientProduit&idProduit=".$produit['P/C-NumID']."');\" class=\"rowClient\">";
				if($produit['P/C-DossierConcurrent'] == 1){
				...
				<div class="panel-body">
					<div class="table-responsive">
						...
						foreach ($anomalies as $ann) {
							...
							$code.="</select></td>
							<td><input type='date' name='date' style='width:120px;height:22px;' value='";
							if($ann['A/P-Date']!=null){$code.=date('Y-m-d',strtotime($ann['A/P-Date']));}
							$code.="'/></td><td>";
							...
							$code.="</td><td><input type='date' style='width:120px;height:22px;' name='dateCloture'  value='";
							if($ann['A/P-DateCloture']!=null){$code.=date('Y-m-d',strtotime($ann['A/P-DateCloture']));}
							$code.="'/></td>
						...
						<tr>
							...
							$code.="</select></td>
							<td><input type='date' style='width:120px;height:22px;' name='date'/></td><td>
							<input name='cloture' type='checkbox'/>
							</td><td><input type='date' style='width:120px;height:22px;' name='dateCloture'/></td>
		...
		<div class="panel panel-primary">
			...
			<div class="panel-body">
				...
				foreach ($maitre as $mai) {
					if(isset($mai['P/C-NumContratMaitre'])){
			...
			<br/><br/><hr/><h4>Evenements liés au produit :</h4>';
				...
				foreach ($evenements as $ev) {
					...
					$code.="&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<b>Date Remise : </b><input style='width:120px;height:22px;' type='date' name='dateRemise' value='";
